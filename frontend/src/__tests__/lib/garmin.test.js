...
describe('formatGarminSummary', () => {
  test('returns null when the session has no imported Garmin activity', () => {
    expect(formatGarminSummary({ title: 'Long run' })).toBeNull()
  })

  test('formats distance, duration, and pace', () => {
    const summary = formatGarminSummary({
      garminImported: true,
