...
**Cloud Functions (`functions/`)** — the one place with server-side code, and only because two things can't live in the browser: a Strava client secret, and a webhook endpoint a personal Shortcuts automation can POST to. `stravaExchangeCode`/`stravaSync` and `generateAppleHealthWebhookUrl` are `onCall` functions (authenticated, invoked from the Training page via the Firebase SDK); `appleHealthWebhook` is a raw `onRequest` HTTP endpoint (see "Fitness data sync" below). Requires the Blaze plan (Spark doesn't support Cloud Functions).
