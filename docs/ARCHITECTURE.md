...
appleHealthTokens/{token}              — { uid, familyId, createdAt }  (nothing else)
```

`trainingProfiles/{uid}` also holds fitness-provider connection state: `stravaConnected` (bool), `terraConnected`/`terraUserId`/`terraProvider` (unused — Terra is shelved, see "Fitness data sync" below), and `appleHealthConnected` (bool) once a webhook URL has been generated at least once. `stravaTokens/{uid}` is a separate collection (rather than fields on the profile) purely so its own Firestore rule can restrict it to `request.auth.uid == uid` alongside `trainingProfiles`, without needing a field-level rule. `appleHealthTokens/{token}` is top-level (not nested under a family) for the same reason `inviteCodes` is: the webhook needs to resolve "which family/uid does this token belong to" from the token alone, before it knows any familyId to scope a query to — a single get-by-exact-id sidesteps that the same way invite-code lookups do. Garmin has no token/credential collection at all — its CSV import is a plain client-side Firestore write through the normal `events` rule, no different from editing an event by hand.
...
## Fitness data sync (Strava, Apple Health, Garmin — Terra shelved)

Training sessions can auto-fill their "actual" distance/duration/pace from a real workout, matched by date to the scheduled session. Every provider writes the same `actualDistanceMiles`/`actualDurationMinutes`/`actualPaceMinPerMile`/`sessionStatus` fields onto the matched event, so the UI doesn't need provider-specific rendering logic beyond a label:
...
- **Terra (tryterra.co) — shelved.** Originally chosen as a unified aggregator to sidestep the Strava/Garmin limits above; the code (`functions/terra.js`, `terraGenerateWidgetSession`/`terraSync`/`terraWebhook`, `lib/terra.js`) is still committed but was never activated once its actual pricing turned out to be $399-499/month with no free tier — not viable for a household hobby project. Left in place mainly as a documented example of the hosted-widget-session + webhook pattern, in case a future integration needs the same shape.
- **Apple Health** — no OAuth, because there's no server-side account to authenticate against at all: HealthKit data lives on-device, and its iCloud copy is end-to-end encrypted (not even Apple can read it). Instead, `generateAppleHealthWebhookUrl` mints a long random per-user token stored in a top-level `appleHealthTokens/{token}` collection (same "unguessable id, admin-SDK-only" pattern as `inviteCodes`) and returns a webhook URL; a personal Shortcuts automation on the user's own phone POSTs a workout as JSON to that URL after each run. `appleHealthWebhook` looks up the token to resolve which family/uid it belongs to, then matches the payload to a scheduled session by date (`functions/appleHealth.js`). There's no signature header to verify here, unlike Terra — the token in the URL is the only authentication, since there's no Apple-issued secret on the other end of this connection.
- **Garmin** — no live sync at all, deliberately. Garmin's own developer program is business-approval-only (and reportedly paused for new applicants in 2026); the unofficial workaround (`python-garminconnect`/`garth`, which logs into Garmin Connect's consumer site with a real password and reuses the session) was considered and rejected — it's outside Garmin's terms of service and would mean storing a friend's actual account password server-side. Instead, `frontend/src/lib/garminImport.js` parses Garmin Connect's own exported Activities CSV entirely client-side and writes matched sessions via a normal Firestore batch write (`applyImportedSessions` in `db.js`) — no Cloud Function, no credentials, since there's no server-side secret involved in reading a file the user already has.
