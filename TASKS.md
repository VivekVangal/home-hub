...
- [ ] **Real-time data / Garmin integration** — Garmin has no self-service personal-use API (Connect Developer Program requires applying as a legal entity/business, manual approval, reportedly paused for new sign-ups as of 2026), and even approved access isn't truly real-time (syncs after upload, not live during a run). Practical paths if this becomes a priority: Strava's API (self-service OAuth, works if Garmin auto-syncs there), manual FIT/CSV import, or the unofficial `python-garminconnect` library (works for personal data, not officially sanctioned by Garmin).
